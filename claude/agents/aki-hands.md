...
|---|---|---|---|
| **Claude subagent (default)** | in-harness spawn of this agent, `model: haiku` for mechanical sweeps | the work is in this repo, the caller wants the result in this session, and the paths are known | nothing — the frontmatter carries the tools, and this body carries the manifest. Name the paths and the question in the prompt |
| **agy headless (Gemini flash)** | `agy --model gemini-3.6-flash-medium --mode plan --output-format json -p "<prompt>"` — **prompt last** | a wide sweep over a large surface, or the Claude quota is the constraint; `--mode plan` makes read-only mechanical rather than worded | `~/.gemini/GEMINI.md` auto-loads the behavior baseline, so the behavior floor is free — but **name the domain rule files and the exact absolute paths anyway**: `cwd` is not a scope boundary for agy |
| **kiro-cli headless** | `kiro-cli chat --no-interactive --trust-tools=fs_read "<prompt>"` | the cheapest bandwidth tier is wanted, or both other quotas are exhausted | everything: no rule file loads by itself. Paste the four sections above plus exact paths. `--trust-tools=fs_read` is the read-only mechanism |
| **cl-9rt (Claude via proxy gateway)** | `CLAUDE_CONFIG_DIR=~/.claude-9rt claude -p --tools "Read,Grep" --model <alias> --effort low "<prompt>"` | a parallel explore lane is wanted **concurrently** with this session, on separate metering | everything, as with kiro. Note the gateway may route an alias to a non-Anthropic core — treat as bandwidth, never as judgment |

**Recorded harness facts — do not re-derive them.** Flags, model names, tiers, read-only mechanisms and known failure modes for every lane above are written down in `~/.claude/skills/akiflow/references/harness-facts.md` (§ Cross-CLI worker · § Kiro CLI · § claude via a proxy gateway). Read that file instead of probing. Running `agy --help`, `agy models`, or a "just checking it works" call to re-learn something already recorded is exactly the redundant work this corpus exists to remove — one drifted session did it three times before using the CLI it had already been told to use.
