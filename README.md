...
A `SessionStart` hook and the installer's `--check` flag both classify install status through one shared parser (`claude/hooks/aki_version_check.py`), which reads the latest released semver — the first `## [x.y.z]` heading, skipping `[Unreleased]`. Five states, one classifier, two consumers:

| State | Condition | Hook (SessionStart) | `install.py --check` |
|---|---|---|---|
| **missing** | no installed `CHANGELOG.md` or `index.md` | announces every session | prints `not installed` + install command |
| **current** | installed semver == remote | silent | prints `up to date` |
| **update** | remote newer than installed | announces version gap + update command | prints the gap |
| **ahead** | installed newer than remote / Unreleased-only | silent | prints `ahead of remote` |
| **unknown** | network/parse failure | silent and retries later | prints `unknown` |

Notify-only: neither surface downloads or installs anything on its own. Windows instructions use `py -3 install.py`; Unix uses `./install.sh`.
...
claude/                           → Claude Code-only runtime assets, installed to ~/.claude/
  ...
  hooks/aki_version_check.py       (shared version-status parser, imported by hook + install.py --check)
...
docs/                             → repo-internal records; one TCC lookup is installed
  ...
  research/                       (event records: frozen body, dated amendments; successor only when Decision changes)
...
flowchart TD
    subgraph SRC["📦 Source: akidevrule Repo"]
        PAYLOAD["payload/ (19 raw rule files)"]
        ...
        CHOOKS["claude/hooks/aki-update-check.py + aki_version_check.py"]
    end
...
1. Syncs `payload/*` into `~/.aki/akidevrule/` (Python `shutil` copy, excludes `ref-ECC/`), removes stale files left by renames, syncs `agskills/` for Antigravity skill inheritance, and deploys `docs/ref/macos-codesign-tcc.md` to the installed lookup tree.
...
7. Installs `~/.claude/hooks/aki-update-check.py` plus shared `~/.claude/hooks/aki_version_check.py`, records the source-repo path in `~/.aki/akidevrule/.source-repo`, and writes `.version` with `installed=` / `version=` / `commit=` / `branch=`.
8. Installs the rendered `payload/GEMINI.md` into the shared corpus and keeps `~/.gemini/GEMINI.md` as a small versioned wrapper that imports it by absolute path, so the global Rule stays under Antigravity's 12,000-character file limit while `GEMINI.local.md` remains machine-local. Generates 18 native rule wrappers under `~/.gemini/config/rules/` with YAML `trigger` frontmatter; each wrapper imports its installed source rule by absolute path instead of duplicating the full body, and current Agy 1.1.x glob rules use one comma-separated scalar `globs` value because its parser rejects a YAML sequence. Deploys 10 skills directly to `~/.gemini/config/skills/` for native auto-discovery, validates their Agent Skills frontmatter before copying, keeps only the canonical absolute shared-root entry in `~/.gemini/config/skills.json`, and merges skill execution permissions into `~/.gemini/antigravity-cli/settings.json` and `~/.gemini/settings.json` — a `command()` prefix rule for each of akiflow's five scripts per skill root, in both the expanded and the tilde-literal command rendering (agy's matcher compares command strings literally; see [docs/ref/cli-permission-allowlist-standard.md](docs/ref/cli-permission-allowlist-standard.md) §1.2) plus scoped `write_file`/`read_file` rules for the council workspace and rule corpus.
