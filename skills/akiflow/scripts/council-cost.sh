...
# exists to avoid.
#
# Usage:  council-cost.sh [transcript.jsonl]
...
if [ -z "$transcript" ]; then
  ...
  if [ ! -d "$dir" ]; then
    echo "no transcript directory: $dir" >&2
    echo "pass the transcript .jsonl path explicitly." >&2
    exit 1
  fi
  transcript="$(ls -t "$dir"/*.jsonl 2>/dev/null | head -1 || true)"
  if [ -z "$transcript" ]; then
    echo "no *.jsonl in $dir" >&2
    exit 1
  fi
fi
...
# uuid -> (isSidechain, parentUuid, first user text on that line)
lines = []
by_uuid = {}
...
def chain_root(d):
    ...
    while True:
        ...
        if not parent.get("isSidechain"):
            return cur.get("uuid")  # first sidechain line under a main-thread parent
        seen.add(p)
        cur = parent
...
# Label each sidechain chain root by scanning its user turns for "You are <NAME>".
root_label = {}
for d in lines:
...
# Aggregate usage.
agents = OrderedDict()   # label -> {model -> counters}
