---
name: akiflow
description: Lead-coordinated council or dispatch for work that needs multiple kinds of judgment or isolated parallel ownership. It pins the owner's verbatim request as the immutable anchor, decomposes quoted requirements into owned items or lanes, convenes only justified seats, separates discuss/audit/execute modes, and uses mechanical closure checks. Explicit invoke only with /akiflow when arbitration or parallel fan-out materially helps; never derive a standing roster from the available agent catalog.
---
