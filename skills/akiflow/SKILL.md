...
| Claude via 9router — parallel lane, not just fallback | `CLAUDE_CONFIG_DIR=~/.claude-9rt claude -p … "<prompt>"` | separate account = separate quota, so it can run **concurrently** with the lead's own session on the same Claude-family model; **gate on `test -d ~/.claude-9rt` first** — the dir is owner-provisioned, not present by default |
