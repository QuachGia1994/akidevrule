...
| **Bandwidth, read-only, off the Claude quota** — a mechanical sweep whose paths can be named up front | cross-CLI worker: `agy --model gemini-3.6-flash-low --mode plan --output-format json -p "<prompt>"` (prompt goes **last** — `-p` takes its value from the next token, so `agy -p --model X "…"` silently sends the wrong thing) | `--mode plan` enforces read-only **by mechanism**, stronger than a Claude subagent's inherited permission mode; `~/.gemini/GEMINI.md` loads the akirule behavior floor into the call for free; the prompt must still name exact paths since `cwd` is not a reliable scope boundary; a denied call still returns `status: "SUCCESS"` with `response: ""`, so an empty response must be treated as a failed call, never a clean sweep; retrieval only — **never judgment** (`references/harness-facts.md` § Cross-CLI worker) |

| **A read-only guarantee that survives a bad prompt** — audit sweeps, anything where "fixing while here" would be unrecoverable | headless self-call: `claude -p --tools "Read,Grep,Glob" --model haiku --effort low --output-format json "<prompt>"` | the Agent tool cannot restrict a spawn's tool set, so a Claude subagent's read-only is prompt wording the model may talk itself out of; `--tools` removes the write tools from the process. Costs more setup than an Agent spawn — use it where the guarantee matters, not everywhere |
| **A result another program will parse**, not a human | add `--json-schema '<schema>'` to either headless form (`claude` and `agy` both accept it) | asking for JSON in prose gets JSON *most* of the time; the flag makes malformed output impossible instead of unlikely |
...
| **Wide, fast discovery over a large surface** — "what exists here", read many files, one answer | `agy --model gemini-3.6-flash-medium --mode plan --output-format json -p "<prompt>"`, **one shot, never a conversation** | ~1M context and the fastest tier available; its failure mode is skimming, not capacity, so the counter is prompt precision — name the exact paths, the exact question, the exact output shape, leave nothing to improvise. Multi-turn on agy degrades badly (measured 2.6s → 8.8s → **57.6s** by turn 3); anything conversational belongs on a Claude session id instead |

**Three worker shapes, and the flags that make them cheap.** Everything below is per-call — a worker never inherits the caller's environment, so cost is chosen at the call site every time:

| Shape | Invocation | Cost levers that matter |
|---|---|---|
| In-session subagent | Agent tool | `model`, `effort` — both mandatory (below) |
| Claude headless | `claude -p … "<prompt>"` | `--model`, `--effort low`, `--tools`, `--max-budget-usd`, `--json-schema`, `--add-dir`, `--no-session-persistence` |
| Cross-CLI headless | `agy … -p "<prompt>"` | `--model`, `--effort`, `--mode plan`, `--json-schema`, `--add-dir` |

Cheapness has **two** axes, not one: the model tier *and* the thinking budget. A mechanical sweep cuts both — `--model haiku --effort low`, not one or the other. A third axis exists but is not the skill's to choose: **which vendor pays**. `agy` can run `claude-sonnet-4-6` and `claude-opus-4-6-thinking` on the Antigravity quota, so model tier and quota are independent decisions. Route by which quota has room, never by which model sounds better — and note that agy's Claude tiers are quota-scarce even on a paid plan, so they are for a single high-value shot, never a habit (`references/harness-facts.md` § Cross-CLI worker).
...
If the owner has shell aliases wrapping these invocations, use them — but the skill names the *shapes*, never a specific alias, because an alias that exists on one machine does not exist on the next. `--bare` is deliberately absent from every recipe above: it is the largest available token cut and it refuses OAuth, so it fails on a normal login (`references/harness-facts.md`).
