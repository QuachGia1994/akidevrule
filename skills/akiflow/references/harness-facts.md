...
| **[owner]** `gemini-3.1-pro` (agy `gemini-3.1-pro-{low,high}`) writes noticeably softer, more natural user-facing prose than Sonnet-class implementers (owner observation across projects, 2026-08-03); its agy quota headroom is unmeasured here. | Default engine for akiflow's writer role (`SKILL.md` Step 7): a role split from the implementer, under an anti-fabrication brief — the writer phrases facts supplied in its brief, never sources its own. Probe quota before a long batch; fallback is a Claude-family writer plus a softening pass. |
