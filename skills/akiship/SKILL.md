...
4. **Push / deploy only if the invocation named them.** If pushed and the stack deploys, run live verification per `release.C5` afterward.
...
- Never push, deploy, or push tags on an invocation that did not name them.
