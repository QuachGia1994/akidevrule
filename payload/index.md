...
| `RULE-agent-engineering.md` | `agent-eng` | Contextual | public | Agent workflow and skill discipline: skill-first/process-before-implementation, shared vocabulary, TDD, root-cause debugging, independent spec/standards review, verification, context-isolated handoffs, validated security findings, and skill self-testing |
...
| `agent-eng` | A Skill selection & workflow order · B Engineering feedback loops · C Agent orchestration & security |
...
| **Agent workflow discipline** (skills, process order, feedback loops, isolated review, validated security) | `agent-eng.A`/`agent-eng.B`/`agent-eng.C` — agent-specific workflow mechanics | `agent.A5` worker economics · `pattern.B2` forest pass · `think.B3` critique · `flow` flow integrity · `zero-trust` evidence classes |
