...
|------|-------|------|------|---------|
| `RULE-agent-behavior.md` | `agent` | Core — `@` import in `~/.claude/CLAUDE.md` | public | §0 penalty cards (`[WRAP]`/`[FLUFF]`/`[YAP]` — one violation vocabulary shared by owner corrections, the `scythe.sh` lint, and akiflow's enforcer), response language, communication-vs-task discrimination (a question is not a request), report-for-fast-reorientation (density, conclusion-first, glossed references), tool-efficiency (every turn re-sends the whole history, so cost follows round-trip count: Read/Edit over Bash print-then-read, all edit sites found then applied in one pass, independent calls batched into one turn, menial work through a worker not personally), worker delegation economics (A5 — default to delegating exploration; wide-context flash tier one-shot for discovery with a fully-specified prompt; stateless-cheap vs. persistent-worker-cheap (`--session-id`, ~⅛ cost from turn 2, cwd-scoped); a worker inherits nothing so name its rule files and targets, and require the return leg — the `[RULES]` receipt naming what it actually received, since "the rule never arrived" and "the rule arrived and was ignored" otherwise share one signature and have opposite fixes; set model tier *and* effort or inherit the expensive default; read-only by mechanism not by wording; structured-output flag when a program parses it; conclusion not dump; judgment never delegates downward; cross-process spend is invisible to the caller), scope discipline, verification and claims (incl. naming a rule is not complying with it — compliance is stated as a checkable fact, never as a citation), decision boundaries, audit-is-read-only (never mutate git state, never auto-classify ambiguous work), no model-credit trailers in git artifacts |
| `RULE-coding.md` | `coding` | Core — `@` import in `~/.claude/CLAUDE.md` | public | Philosophy, source-of-truth, verification (narrowest tool; app runs are user-triggered; runtime-only risk that can't run → "unverified" handoff, never a false "Done"), self-documenting code & comment budget (naming first, deletion test, comment rot — a stale comment misleads worse than none), error handling, security |
| `RULE-design-core.md` | `design` | Core — `@` import in `~/.claude/CLAUDE.md` | public | Universal pattern philosophy: SSoT, Rule of Three, SRP "and"-test, OCP, composition, module boundaries, name-by-role, anti-patch. Sharpens RULE-coding; applies to every project type — load eagerly on any structural/decomposition decision |
| `RULE-docs.md` | `docs` | Contextual | public | Docs structure (incl. mandatory `docs/biz/` backbone), plan lifecycle, research doc schema (immutable event record: start time/purpose/strategy/checklist/result+verification/decision+cross-refs), doc-sync behavior, drift audit (when it runs vs the two situations it does not, research+plan doc pair, comparison checklist, wrong/stale/incomplete/cosmetic severity) |
| `RULE-content-write.md` | `content` | Contextual | public | UI copy, semantic stability, writing style (density enforced by deletion test), i18n |
...
| Topic | Groups |
|---|---|
| `agent` | §0 Penalty cards · A Giao tiếp · B Kỷ luật phạm vi & quyết định · C File & bộ nhớ |
