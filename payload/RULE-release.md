...
- **Completion-intensity phrasing collapses condition (2), never (1) or (3).** A completion-intensity signal in the invocation — "trọn vẹn", "hoàn thành"/"hoàn thiện", "làm/xong hết", "tất cả"/"toàn bộ", or equivalent sentiment insisting the run finish everything, end to end — resolves B7 step 0's mid-edit-vs-abandoned ambiguity toward **mid-edit by default**: finish and integrate the leftover instead of stopping to ask. Conditions (1) and (3) gate on irreversibility (a published-version rewrite) and correctness (a documented-design contradiction), not on effort, so no phrasing intensity waives them — a "nghiêm trọng"/major-contradiction hit still stops the run.
