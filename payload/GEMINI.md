...
> Managed by akidevrule `install.sh`. Do NOT hand-edit — changes are lost on next install. Machine-specific facts (local paths, CLIs, emulators) go in `~/.gemini/GEMINI.local.md`; the generated global wrapper loads this managed source first, then appends the local file verbatim. The marker on line 1 is the install-version fingerprint; a project bootstrap file uses it to detect whether these overrides are present.
