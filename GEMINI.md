...
> 1. Read `~/.aki/claudedoc/.source-repo` — it holds this machine's absolute path to the akidevrule source repo. Then run `bash <that path>/install.sh` **without asking**: it backs up every file it touches and never overwrites `*.local.md`, so it is reversible and inside your mandate. Asking here is friction, not safety.
