...
backup() {
  if [ -e "$1" ]; then
    cp -R "$1" "$1.akidevrule-backup-$STAMP"
  fi
}
...
prune_backups() {
  ...
  name="$(basename "$base")"
  local count
  count=$(find "$dir" -maxdepth 1 -name "${name}.akidevrule-backup-*" | wc -l | tr -d ' ')
  if [ "$count" -le 2 ]; then
    return
  fi
  find "$dir" -maxdepth 1 -name "${name}.akidevrule-backup-*" | sort | head -n "$((count - 2))" | while IFS= read -r f; do
...
if [ "$had_unmanaged_gemini" = true ]; then
  echo -e "  ⚠️  Your previous ~/.gemini/GEMINI.md was replaced (saved as *.akidevrule-backup-*)."
...
while IFS='|' read -r rule_file rule_trigger rule_desc rule_globs; do
...

  {
    ...
    echo "---"
    echo ""
    echo "<!-- Generated by akidevrule install.sh from payload/$rule_file. Do not edit here. -->"
    echo ""
    cat "$src"
  } > "$dest"
